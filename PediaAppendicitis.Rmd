...
- In severe cases, abscess can be seen and this Figure shows that the proportion is higher for complicated cases.
...
- Since the use of ultrasound is less expensive and less harmful than CT, we will use ultrasound data here to see the impact on the diagnostic process.
...
ci2 |>
  kable(
    digits = 2,
    caption = "95% confidence interval for the difference in mean of appendiceal diameter between \n patients diagnosed with appendicitis or no appendicitis."
  )
```
...
- To help the health workers make more informed decisions (i.e, Accurately diagnosing the appendicitis) we would use a supervised machine learning.
